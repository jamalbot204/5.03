...
const ChatMessageList = memo(forwardRef<ChatMessageListHandles, ChatMessageListProps>(({ onEnterReadMode }, ref) => {
    ...
    // 3b. SMART STICKINESS FOR STREAMING: Subscribe to streamingText changes without re-rendering
    useEffect(() => {
        const unsubscribe = useStreamingStore.subscribe(
            (state, prevState) => {
                if (state.streamingText !== prevState.streamingText) {
                    if (isAtBottomRef.current && !hasInterruptedScrollRef.current && visibleMessages.length > 0) {
                        requestAnimationFrame(() => {
                            if (!hasInterruptedScrollRef.current) {
                                virtualizer.scrollToIndex(visibleMessages.length - 1, { align: 'end' });
                            }
