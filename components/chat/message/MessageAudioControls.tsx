...
const MessageAudioControls: React.FC<MessageAudioControlsProps> = memo(({ 
    ...
    const renderPlayButtonForSegment = (partIndexInput?: number) => {
        ...
        if (isStreaming) { IconComponent = SpeakerWaveIcon; isPulsing = true; isDisabled = true; title = isMainContextButton ? t.loadingAudio : `${t.loadingPart} ${partIndexInput! + 1}...`; iconClassName = 'text-blue-400';
        } else if (isThisTheMainButtonOverallFetching) { IconComponent = XCircleIcon; iconClassName = 'text-red-400 hover:text-red-300'; title = `${t.cancelFetching} ${textSegmentsForTts.length}`; isPulsing = true;
